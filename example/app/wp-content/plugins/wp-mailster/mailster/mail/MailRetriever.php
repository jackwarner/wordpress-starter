...
class MstMailRetriever
{
	...
	function retrieveMailsOfActiveMailingLists($minDuration, $execEnd){
		...
		$mailingLists = $listUtils->getActiveMailingLists(true, true);
		...
		for($i = 0; $i < $nrLists; $i++) {		
			...
			if($timeLeft > $minDuration){
				...
				$log->debug('Timeout while retrieving mails of active mailing lists, time left to run: ' . $timeLeft . ' sec (min. duration is ' . $minDuration.' sec)');
				break;
			}
		}
